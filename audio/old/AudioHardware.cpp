...
enum {
    INVALID_DEVICE                        = 65535,
    SND_DEVICE_CURRENT                    = -1,
    SND_DEVICE_HANDSET                    = 0,
    SND_DEVICE_SPEAKER                    = 1,
    SND_DEVICE_HEADSET                    = 2,
    SND_DEVICE_FM_HANDSET                 = 3,
    SND_DEVICE_FM_SPEAKER                 = 4,
    SND_DEVICE_FM_HEADSET                 = 5,
    SND_DEVICE_BT                         = 6,
    SND_DEVICE_HEADSET_AND_SPEAKER        = 7,
    SND_DEVICE_NO_MIC_HEADSET             = 8,
    SND_DEVICE_IN_S_SADC_OUT_HANDSET      = 9,
    SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE= 10,
    SND_DEVICE_TTY_HEADSET                = 11,
    SND_DEVICE_TTY_HCO                    = 12,
    SND_DEVICE_TTY_VCO                    = 13,
    SND_DEVICE_TTY_FULL                   = 14,
    SND_DEVICE_HDMI                       = 15,
    SND_DEVICE_CARKIT                     = -1,
    SND_DEVICE_ANC_HEADSET                = 16,
    SND_DEVICE_NO_MIC_ANC_HEADSET         = 17,
    SND_DEVICE_HEADPHONE_AND_SPEAKER      = 18,
    SND_DEVICE_FM_TX                      = 19,
    SND_DEVICE_FM_TX_AND_SPEAKER          = 20,
    SND_DEVICE_SPEAKER_TX                 = 21,
    SND_DEVICE_BACK_MIC_CAMCORDER         = 33,
    SND_DEVICE_BT_EC_OFF                  = -1,
};

enum {
    DEVICE_HANDSET_RX            = 0, // handset_rx
    DEVICE_HANDSET_TX            = 1,//handset_tx
    DEVICE_SPEAKER_RX            = 2, //speaker_stereo_rx
    DEVICE_SPEAKER_TX            = 3,//speaker_mono_tx
    DEVICE_HEADSET_RX            = 4, //headset_stereo_rx
    DEVICE_HEADSET_TX            = 5, //headset_mono_tx
    DEVICE_FMRADIO_HANDSET_RX    = 6, //fmradio_handset_rx
    DEVICE_FMRADIO_HEADSET_RX    = 7, //fmradio_headset_rx
    DEVICE_FMRADIO_SPEAKER_RX    = 8, //fmradio_speaker_rx
    DEVICE_DUALMIC_HANDSET_TX    = 9, //handset_dual_mic_endfire_tx
    DEVICE_DUALMIC_SPEAKER_TX    = 10, //speaker_dual_mic_endfire_tx
    DEVICE_TTY_HEADSET_MONO_RX   = 11, //tty_headset_mono_rx
    DEVICE_TTY_HEADSET_MONO_TX   = 12, //tty_headset_mono_tx
    DEVICE_SPEAKER_HEADSET_RX    = 13, //headset_stereo_speaker_stereo_rx
    DEVICE_FMRADIO_STEREO_TX     = 14,
    DEVICE_HDMI_STERO_RX         = 15, //hdmi_stereo_rx
    DEVICE_ANC_HEADSET_STEREO_RX = 16, //ANC RX
    DEVICE_BT_SCO_RX             = 17, //bt_sco_rx
    DEVICE_BT_SCO_TX             = 18, //bt_sco_tx
    DEVICE_FMRADIO_STEREO_RX     = 19,
    DEVICE_CAMCORDER_TX          = 105, // camcoder_tx (misspelled by Samsung)
};

enum {
    FLUENCE_MODE_ENDFIRE   = 0,
    FLUENCE_MODE_BROADSIDE = 1,
};
...
int enableANC(int enable, uint32_t device)
{
    ...
    {
        ALOGD("ANC driver is already in state %d. Not calling anc driver", enable);
        return -EPERM;
    }
    ...
    if ( rc == 0 )
    {
        ALOGD("msm_enable_anc was successful");
        anc_enabled = enable;
    } else
    {
        ALOGD("msm_enable_anc failed");
    ...
    while(temp_head->next != NULL) {
        ...
        switch(temp_ptr->stream_type) {
            ...
            case FM_A2DP:
                ...
                ALOGD("rx_device = %d,temp_ptr->dev_id = %d",rx_device,temp_ptr->dev_id);
                ...
                if(msm_route_stream(PCM_PLAY,temp_ptr->dec_id,DEV_ID(temp_ptr->dev_id),0)) {
                     ALOGD("msm_route_stream(PCM_PLAY,%d,%d,0) failed",temp_ptr->dec_id,DEV_ID(temp_ptr->dev_id));
                }
                ...
                if(msm_route_stream(PCM_PLAY,temp_ptr->dec_id,DEV_ID(rx_device),1)) {
                    ALOGD("msm_route_stream(PCM_PLAY,%d,%d,1) failed",temp_ptr->dec_id,DEV_ID(rx_device));
                }
            ...
            case PCM_REC:
                ...
                if(msm_route_stream(PCM_REC,temp_ptr->dec_id,DEV_ID(tx_device),1)) {
                    ALOGD("msm_route_stream(PCM_REC,%d,%d,1) failed",temp_ptr->dec_id,DEV_ID(tx_device));
                }
...
#endif
{
    ...
    char value[128];
        ...
        ALOGD("got device_count %d",dev_cnt);
        if (dev_cnt <= 0) {
        ...
        for(i = 0; i < dev_cnt;i++) {
            if(strcmp((char* )name[i],"handset_rx") == 0) {
                ...
                if (fluence_mode == FLUENCE_MODE_ENDFIRE) {
                     ...
                     ALOGD("Endfire handset found but user request for %d\n", fluence_mode);
                     continue;
                }
            }
                ...
                if (fluence_mode == FLUENCE_MODE_ENDFIRE) {
                     ...
                     ALOGD("Endfire speaker found but user request for %d\n", fluence_mode);
                     continue;
                }
            }
                ...
                if (fluence_mode == FLUENCE_MODE_BROADSIDE) {
                     ...
                     ALOGD("Broadside handset found but user request for %d\n", fluence_mode);
                     continue;
                }
            }
                ...
                if (fluence_mode == FLUENCE_MODE_BROADSIDE) {
                     ...
                     ALOGD("Broadside speaker found but user request for %d\n", fluence_mode);
                     continue;
                }
            }
        ...
        uint32_t devices, int *format, uint32_t *channels, uint32_t *sampleRate, status_t *status)
{
    ...
    { // scope for the lock
        ...
        if ((flags & AUDIO_OUTPUT_FLAG_DIRECT) && (flags & AUDIO_OUTPUT_FLAG_VOIP_RX)) {
            ...
            if(mDirectOutput == 0) {
               ALOGD(" AudioHardware::openOutputStream Direct output stream \n");
...
void AudioHardware::closeInputStream(AudioStreamIn* in) {
    ...
    if((index = mInputs.indexOf((AudioStreamInMSM8x60 *)in)) >= 0) {
        ALOGD("closeInputStream AudioStreamInMSM8x60");
        mLock.unlock();
        ...
        ALOGD("closeInputStream mVoipInputs");
        mLock.unlock();
...
String8 AudioHardware::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioHardware::getParameters() %s", param.toString().string());
...
#endif
    ALOGD("msm_set_voice_rx_vol(%d) succeeded session_id %d",vol,session_id);
    return NO_ERROR;
}
...
status_t AudioHardware::setFmVolume(float v)
{
    ...
    if(temp == NULL){
        ALOGD("No Active FM stream is running");
        return NO_ERROR;
    }
    ...
    ALOGD("msm_set_volume(%d) for FM succeeded",vol);
    return NO_ERROR;
}
#endif
                                   ...
                                   int mode, bool mic_mute)
{
    ...
    Routing_table* temp = NULL;
    ALOGD("do_route_audio_rpc(%d, %d, %d)", device, mode, mic_mute);
    ...
    if(device == SND_DEVICE_HANDSET) {
        ...
        ALOGD("In DUALMIC_HANDSET");
        if(DEV_ID(new_tx_device) == INVALID_DEVICE) {
            new_tx_device = DEVICE_HANDSET_TX;
            ALOGD("Falling back to HANDSET_RX AND HANDSET_TX as no DUALMIC_HANDSET_TX support found");
        }
    }
    else if(device == SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE) {
        new_rx_device = DEVICE_SPEAKER_RX;
        new_tx_device = DEVICE_DUALMIC_SPEAKER_TX;
        ALOGD("In DUALMIC_SPEAKER");
        if(DEV_ID(new_tx_device) == INVALID_DEVICE) {
            new_tx_device = DEVICE_SPEAKER_TX;
            ALOGD("Falling back to SPEAKER_RX AND SPEAKER_TX as no DUALMIC_SPEAKER_TX support found");
        }
    }
        ...
        new_tx_device = DEVICE_HEADSET_TX;
        ALOGD("In DEVICE_SPEAKER_HEADSET_RX and DEVICE_HEADSET_TX");
        if(DEV_ID(new_rx_device) == INVALID_DEVICE) {
             new_rx_device = DEVICE_HEADSET_RX;
             ALOGD("Falling back to HEADSET_RX AND HEADSET_TX as no combo device support found");
        }
    }
    else if(device == SND_DEVICE_HEADPHONE_AND_SPEAKER) {
        new_rx_device = DEVICE_SPEAKER_HEADSET_RX;
        new_tx_device = DEVICE_HANDSET_TX;
        ALOGD("In DEVICE_SPEAKER_HEADSET_RX and DEVICE_HANDSET_TX");
        if(DEV_ID(new_rx_device) == INVALID_DEVICE) {
             new_rx_device = DEVICE_HEADSET_RX;
             ALOGD("Falling back to HEADSET_RX AND HANDSET_TX as no combo device support found");
        }
    }
    ...
    if ((mode == AUDIO_MODE_IN_CALL) && !isStreamOn(VOICE_CALL)) {
        ...
        msm_start_voice();
#endif
        ALOGD("Going to enable RX/TX device for voice stream");
            // Routing Voice
        ...
        ALOGD("Going to disable RX/TX device during end of voice call");
...
// the code and not removing it.
    ...
    ALOGD("doAudioRouteOrMute() device %x, mMode %d, mMicMute %d", device, mMode, mMicMute);
    ...
    if(!getNodeByStreamType(VOICE_CALL)
      ...
      && !getNodeByStreamType(LPA_DECODE)
        ...
        if(enableDevice(cur_rx, 0)) {
            ALOGD("Disable device[%d] failed errno = %d",DEV_ID(cur_rx),errno);
            return 0;
        }
    }
    return NO_ERROR;
...
{
    ALOGD("setupDeviceforVoipCall value %d",value);
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    if(mHardware->mVoipFd >= 0) {
        ...
        {
            ALOGD("Starting voip on Rx %d and Tx %d device", DEV_ID(cur_rx), DEV_ID(cur_tx));
        ...
        mFd = status;
        ALOGD("VOPIstreamin : Save the fd %d \n",mFd);
        mHardware->mVoipFd = mFd;
        // Increment voip stream count

        // configuration
        ALOGD("get mvs config");
    ...
    ALOGD(" AudioHardware::AudioStreamInVoip::set after configuring devices\
            = %u format = %d pChannels = %u Rate = %u \n",
             devices, mFormat, mChannels, mSampleRate);

    ALOGD(" Set state  AUDIO_INPUT_OPENED\n");
...
AudioHardware::AudioStreamInVoip::~AudioStreamInVoip()
{
    ALOGD("AudioStreamInVoip destructor");
...
{
//    ALOGD("AudioStreamInVoip::read(%p, %ld)", buffer, bytes);
    ...
    if (mState < AUDIO_INPUT_OPENED || (mHardware->mVoipFd == -1)) {
      ...
      ALOGD("AudioStreamInVoip::read : device is already open \n");
    }
    ...
    audio_mvs_frame.frame_type = 0;
       while (count >= mBufferSize) {
           ...
           ALOGD("Calling read count = %u mBufferSize = %u \n", count, mBufferSize);
           int bytesRead = ::read(mFd, &audio_mvs_frame, sizeof(audio_mvs_frame));
           ALOGD("PCM read_bytes = %d mvs\n", bytesRead);
           if (bytesRead > 0) {
...
status_t AudioHardware::AudioStreamInVoip::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    ALOGD("AudioStreamInVoip::setParameters() %s", keyValuePairs.string());
...
String8 AudioHardware::AudioStreamInVoip::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioStreamInVoip::getParameters() %s", param.toString().string());
    return param.toString();
}
...
AudioHardware::AudioStreamOutMSM8x60::AudioStreamOutMSM8x60() :
    ...
    mFormat(AUDIO_FORMAT_PCM_16_BIT), mChannels(AUDIO_CHANNEL_OUT_STEREO)
{
...
status_t AudioHardware::AudioStreamOutMSM8x60::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    ALOGD("AudioStreamOutMSM8x60::setParameters() %s", keyValuePairs.string());

    if (param.getInt(key, device) == NO_ERROR) {
        mDevices = device;
        ALOGD("set output routing %x", mDevices);
...
String8 AudioHardware::AudioStreamOutMSM8x60::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioStreamOutMSM8x60::getParameters() %s", param.toString().string());
    return param.toString();
}
...
AudioHardware::AudioStreamOutDirect::~AudioStreamOutDirect()
{
    ALOGD("AudioStreamOutDirect destructor");
...
//    ALOGE("AudioStreamOutDirect::write(%p, %u)", buffer, bytes);
    ...
    if (mStandby) {
        if(mHardware->mVoipFd >= 0) {
            ...
            if ((cur_rx != INVALID_DEVICE) && (cur_tx != INVALID_DEVICE))
            {
                ALOGD("Starting voip call on Rx %d and Tx %d device", DEV_ID(cur_rx), DEV_ID(cur_tx));
                msm_route_voice(DEV_ID(cur_rx),DEV_ID(cur_tx), 1);
            }
            else {
               return -1;
    ...
    if (mFormat == AudioSystem::PCM_16_BIT) {
        ...
        ALOGD("Write Frametype %d, Frame len %d", audio_mvs_frame.frame_type, audio_mvs_frame.len);
        if(audio_mvs_frame.len < 0)
            goto Error;
        memcpy(&audio_mvs_frame.voc_pkt, &mvsFramePtr->voc_pkt, audio_mvs_frame.len);
        size_t written =::write(mFd, &audio_mvs_frame, sizeof(audio_mvs_frame));
        ALOGD(" mvs bytes written : %d bytes %d \n", written,bytes);
    }   
...
status_t AudioHardware::AudioStreamOutDirect::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    ALOGD("AudioStreamOutDirect::setParameters() %s", keyValuePairs.string());
...
String8 AudioHardware::AudioStreamOutDirect::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioStreamOutDirect::getParameters() %s", param.toString().string());
    return param.toString();
}
                                         ...
                                         status_t   *status)
{
    ...
    *status             = BAD_VALUE;
    ...
    mInputBufferCount   = LPA_BUFFER_COUNT;
    efd = -1;
...
{
    ALOGD("AudioSessionOutLPA destructor");
	mSkipWrite = true;
    mWriteCv.signal();
...
ssize_t AudioHardware::AudioSessionOutLPA::write(const void* buffer, size_t bytes)
{
    Mutex::Autolock autoLock(mLock);
    int err;
    ALOGD("write Empty Queue size() = %d, Filled Queue size() = %d ",
    ...
    if ( buf.bytesToWrite > 0) {
        ...
        if ( (buf.bytesToWrite % 2) != 0 ) {
            ALOGD("Increment for even bytes");
        ...
        if (timeStarted == 0)
            ...
            ALOGD("mEmptyQueueMutex locking: %d", __LINE__);
            mEmptyQueueMutex.lock();
            ALOGD("mEmptyQueueMutex locked: %d", __LINE__);
            mEmptyQueue.push_back(buf);
            ALOGD("mEmptyQueueMutex unlocking: %d", __LINE__);
            mEmptyQueueMutex.unlock();
            ALOGD("mEmptyQueueMutex unlocked: %d", __LINE__);
        ...
        if (mFilledQueue.empty() && !mEosEventReceived) {
            ALOGD("mEosEventReceived made true");
...
String8 AudioHardware::AudioSessionOutLPA::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioSessionOutLPA::getParameters() %s", param.toString().string());
    return param.toString();
}
...
status_t AudioHardware::AudioSessionOutLPA::setVolume(float left, float right)
{
    ...
    float vol = v * 100;
    ALOGD("AudioSessionOutLPA::setVolume(%f)\n", v);
    ALOGD("Setting session volume to %f (available range is 0 to 100)\n", vol);

    if(msm_set_volume(sessionId, vol)) {
        ALOGE("msm_set_volume(%d %f) failed errno = %d",sessionId, vol,errno);
        return -1;
    }
    ALOGD("LPA volume set (%f) succeeded",vol);
    return NO_ERROR;
}
...
status_t AudioHardware::AudioSessionOutLPA::openAudioSessionDevice( )
{
    ...
    if ( afd < 0 ) {
        ...
        //initCheck = true;
        ALOGD("pcm_lp_dec: pcm_lp_dec Driver opened");
    }
...
void* AudioHardware::AudioSessionOutLPA::memBufferAlloc(int nSize, int32_t *ion_fd)
{
    ...
    BuffersAllocated buf(ion_buf, nSize, fd_data.fd, alloc_data.handle);
    mEmptyQueue.push_back(buf);
    mBufPool.push_back(buf);

    // 4. Send the mem fd information
    *ion_fd = fd_data.fd;
    ALOGD("IONBufferAlloc calling with required size %d", nSize);
    ALOGD("ION allocated is %d, fd_data.fd %d and buffer is %x", *ion_fd, fd_data.fd, (unsigned int)ion_buf);
...
void AudioHardware::AudioSessionOutLPA::bufferDeAlloc()
{
    ...
    while (!mEmptyQueue.empty())  {
        ...
        ALOGD("Ion Unmapping the address %p, size %d, fd %d from empty",ionBuffer.memBuf,ionBuffer.bytesToWrite,ionBuffer.memFd);
        munmap(ionBuffer.memBuf, mInputBufferSize);
        ALOGD("closing the ion shared fd");
    ...
    while(!mFilledQueue.empty()){
        ...
        ALOGD("Ion Unmapping the address %p, size %d, fd %d from Request",ionBuffer.memBuf,ionBuffer.bytesToWrite,ionBuffer.memFd);
        munmap(ionBuffer.memBuf, mInputBufferSize);
        ALOGD("closing the ion shared fd");
        close(ionBuffer.memFd);
        rc = ioctl(ionfd, ION_IOC_FREE, &ionBuffer.ion_handle);
...
void  AudioHardware::AudioSessionOutLPA::eventThreadEntry()
{
    ...
    while (!mKillEventThread) {
        ...
        if ( (rc < 0) && ((errno == ENODEV) || (errno == EBADF)) ) {
            ALOGD("AUDIO__GET_EVENT called. Exit the thread");
            break;
        }
        ...
        case AUDIO_EVENT_SUSPEND:
            {
                ...
                ALOGD("AUDIO_EVENT_SUSPEND received\n");
                if (!mPaused) {
                    ALOGD("Not in paused, no need to honor SUSPEND event");
                    break;
                }
        ...
        case AUDIO_EVENT_RESUME:
            {
                ALOGD("AUDIO_EVENT_RESUME received\n");
            }
            break;
        default:
...
status_t AudioHardware::AudioSessionOutLPA::flush()
{
    ...
    mEmptyQueueMutex.unlock();
    mFilledQueueMutex.unlock();
    ALOGD("Transferred all the buffers from Filled queue to "
...
AudioHardware::AudioSessionOutTunnel::AudioSessionOutTunnel( AudioHardware *hw,
                                         ...
                                         status_t   *status)
{
    ALOGD("AudioSessionOutTunnel constructor");
    ...
    *status             = BAD_VALUE;
    ...
    mChannels           = popcount(channels);
    mChannelMask        = channels;
    mBufferSize         = TUNNEL_BUFFER_SIZE; //TODO to check what value is correct
...
{
    ALOGD("AudioSessionOutTunnel destructor");
    mSkipWrite = true;
    mWriteCv.signal();
    reset();
}
...
ssize_t AudioHardware::AudioSessionOutTunnel::write(const void* buffer, size_t bytes)
{
    Mutex::Autolock autoLock(mLock);
    int err;
    ALOGD("AudioSessionOutTunnel::write for bytes %d QueueStats empty q %d filled q %d",
         bytes, mEmptyQueue.size(), mFilledQueue.size());

    mEmptyQueueMutex.lock();

    if (mEmptyQueue.empty()) {
        ALOGD("AudioSessionOutTunnel::write: waiting for want of buffers");
        mLock.unlock();
        mWriteCv.wait(mEmptyQueueMutex);
        mLock.lock();
        if (mSkipWrite) {
            ALOGD("AudioSessionOutTunnel::write: Skipping the previous write buffer");
            mSkipWrite = false;
            mEmptyQueueMutex.unlock();
            return 0;
        }
        ALOGD("AudioSessionOutTunnel::write: received a signal to wake up");
    }
    ...
    if (bytes > 0) {
        ...
        if ((buf.bytesToWrite % 2) != 0) {
            ALOGD("AudioSessionOutTunnel::writeIncrement for even bytes");
            aio_buf_local.data_len += 1;
        }
    } else {
    ...
    ALOGD("AudioSessionOutTunnel::write Buffer: QueueStats empty q %d filled q %d %d"
    ...
    ALOGD("AudioSessionOutTunnel::Write Buffer done.");
    mFlushLock.unlock();
...
status_t AudioHardware::AudioSessionOutTunnel::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    ALOGD("AudioSessionOutTunnel::setParameters() %s", keyValuePairs.string());
...
String8 AudioHardware::AudioSessionOutTunnel::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioSessionOutTunnel::getParameters() %s", param.toString().string());
    return param.toString();
}
...
status_t AudioHardware::AudioSessionOutTunnel::setVolume(float left, float right)
{
    ...
    float vol = v * 100;
    ALOGD("AudioSessionOutTunnel::setVolume(%f)\n", v);
    ALOGD("Setting session volume to %f (available range is 0 to 100)\n", vol);
    ...
    ALOGD("AudioSessionOutTunnel:msm_set_volume(%f) succeeded",vol);
    return NO_ERROR;
}
...
{
    status_t status = NO_ERROR;
    const char *drvr=NULL;

    switch (mFormat & AUDIO_FORMAT_MAIN_MASK) {
    case AUDIO_FORMAT_PCM:
        drvr = "/dev/msm_pcm_lp_dec";
        break;
    ...
    case AUDIO_FORMAT_AAC:
        ...
        break;
    default:
        ALOGE("AudioSessionOutTunnel::initSession:Tunnel session supported only for PCM/MP3/AAC");
        return -1;
    }
    ...
    if (afd < 0) {
        ...
        ALOGD("AudioSessionOutTunnel::initSession:Tunnel mode Drvr Opened");
    }
...
void* AudioHardware::AudioSessionOutTunnel::memBufferAlloc(int nSize, int32_t *ion_fd)
{
    ...
    int rc = ioctl(ionfd, ION_IOC_ALLOC, &alloc_data);
    if (rc) {
        ALOGE("ION_IOC_ALLOC ioctl failed with %d\n", rc);
        return ion_buf;
    }
    fd_data.handle = alloc_data.handle;
    ...
    rc = ioctl(ionfd, ION_IOC_SHARE, &fd_data);
    if (rc) {
        ALOGE("ION_IOC_SHARE ioctl failed with %d\n", rc);
        rc = ioctl(ionfd, ION_IOC_FREE, &(alloc_data.handle));
        if (rc) {
            ALOGE("ION_IOC_FREE ioctl failed with %d\n", rc);
        }
        return ion_buf;
    }
...
void  AudioHardware::AudioSessionOutTunnel::eventThreadEntry()
{
    ...
    androidSetThreadPriority(tid, ANDROID_PRIORITY_AUDIO);
    prctl(PR_SET_NAME, (unsigned long)"HAL Audio EventThread", 0, 0, 0);
    ALOGD("AudioSessionOutTunnel::event thread: created ");
    if (mKillEventThread) {
        mEventThreadAlive = false;
        ALOGD("AudioSessionOutTunnel::event thread: is dying.");
        return;
    }
    while (1) {
        ...
        if ( (rc < 0) && ((errno == ENODEV) || (errno == EBADF) ) ) {
            ALOGD("AudioSessionOutTunnel::event thread: Exit the thread");
            break;
        }
        ...
        case AUDIO_EVENT_WRITE_DONE:
            {
                ALOGD("AudioSessionOutTunnel::event thread:WRITE_DONE: addr %p len %d and fd is %d\n",
                     ...
                     (int32_t) cur_pcmdec_event.event_payload.aio_buf.private_data);
                mFilledQueueMutex.lock();
                ALOGD("AudioSessionOutTunnel::event thread:Write Buffer Done: QueueStats empty q %d filled q %d"
                    , mEmptyQueue.size(), mFilledQueue.size());
                for (List<BuffersAllocated>::iterator it = mFilledQueue.begin();
                    it != mFilledQueue.end(); ++it) {
                    if (it->memBuf == cur_pcmdec_event.event_payload.aio_buf.buf_addr) {
                        ...
                        mEmptyQueueMutex.unlock();
                        ALOGD("mEmptyQueueMutex unlocked: %d", __LINE__);
                        ALOGD("Write Buffer Done: QueueStats empty q %d filled q %d %d"
                            , mEmptyQueue.size(), mFilledQueue.size(), __LINE__);
                        mWriteCv.signal();
                        break;
                    }
                ...
                if (mCallback) {
                    callback(STREAM_CBK_EVENT_WRITE_READY, mCallbackCookie);
                }
            }
            break;
        case AUDIO_EVENT_SUSPEND:
            {
                ...
                ALOGD("AudioSessionOutTunnel::event thread:AUDIO_EVENT_SUSPEND received\n");
                if (!mPaused) {
                    ALOGD("AudioSessionOutTunnel::event thread:Not in paused, no need to honor SUSPEND event");
                    break;
                }
                // 1. Get the Byte count that is consumed
                if ( ioctl(afd, AUDIO_GET_STATS, &stats)  < 0 ) {
                    ...
                    ALOGD("AudioSessionOutTunnel::event thread:Number of bytes consumed by DSP is %u", stats.byte_count);
                    nBytesConsumed = stats.byte_count;
                    }
                    // 3. Call AUDIO_STOP on the Driver.
                    ALOGD("AudioSessionOutTunnel::event thread:Received AUDIO_EVENT_SUSPEND and calling AUDIO_STOP");
                    if ( ioctl(afd, AUDIO_STOP, 0) < 0 ) {
                         ALOGE("AUDIO_STOP failed");
                    }
                    break;
        ...
        case AUDIO_EVENT_RESUME:
            {
                ALOGD("AudioSessionOutTunnel::event thread:AUDIO_EVENT_RESUME received\n");
            }
            break;
        default:
...
status_t AudioHardware::AudioSessionOutTunnel::start( )
{
    ...
    ALOGD("AudioSessionOutTunnel::start Tunnel playback start");
    if (mPaused) {
        if (ioctl(afd, AUDIO_PAUSE, 0) < 0) {
        ...
        if (ioctl(afd, AUDIO_GET_SESSION_ID, &decId) == -1) {
            ...
            sessionId = (int)decId;
            ALOGD("AudioSessionOutTunnel::start:AUDIO_GET_SESSION_ID success : decId = %d", decId);
        }
        ...
        ALOGD("AudioSessionOutTunnel::start:AudioSessionOutTunnel::set() Adding LPA_DECODE Node to Table");
        addToTable(sessionId,cur_rx,INVALID_DEVICE,LPA_DECODE,true);
#endif
        ALOGD("AudioSessionOutTunnel::start:enableDevice(cur_rx = %d, dev_id = %d)",cur_rx,DEV_ID(cur_rx));
        if (enableDevice(cur_rx, 1)) {
            ALOGE("AudioSessionOutTunnel::start:enableDevice failed for device cur_rx %d", cur_rx);
            return -1;
        }

        ALOGD("AudioSessionOutTunnel::start:msm_route_stream(PCM_PLAY,%d,%d,0)",sessionId,DEV_ID(cur_rx));
#ifdef QCOM_ACDB_ENABLED
        acdb_loader_send_audio_cal(ACDB_ID(cur_rx), CAPABILITY(cur_rx));
#endif
        if(msm_route_stream(PCM_PLAY,sessionId,DEV_ID(cur_rx),1)) {
...
{
    ALOGD("AudioSessionOutTunnel::pause:Tunnel playback pause");
    if (ioctl(afd, AUDIO_PAUSE, 1) < 0) {
...
status_t AudioHardware::AudioSessionOutTunnel::flush()
{
    ...
    mEmptyQueueMutex.lock();
    List<BuffersAllocated>::iterator it = mBufPool.begin();
    ALOGD("AudioSessionOutTunnel::flush : QueueStats empty q %d filled q %d %d"
        , mEmptyQueue.size(), mFilledQueue.size(), __LINE__);
...
{
    Mutex::Autolock autoLock(mLock);
    ALOGD("AudioSessionOutTunnel- stop");
    // close all the existing PCM devices
    mSkipWrite = true;
    mWriteCv.signal();
...
status_t AudioHardware::AudioSessionOutTunnel::set_callback(stream_callback_t callback, void *cookie)
{
    mCallback = callback;
...
status_t AudioHardware::AudioSessionOutTunnel::get_presentation_position(uint64_t *frames, struct timespec *timestamp);
{
    if (frames) {
        msm_audio_stats stats;
        uint64_t time;
        ...
        ioctl(afd, AUDIO_GET_STATS, &stats);
        time = stats.unused[0] | ((uint64_t)stats.unused[1] << 32);
        ALOGD("Timestamp returned = %lld\n", time);
        *frames = time * sampleRate() / 1000000;
    }
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    ALOGD("AudioStreamInMSM8x60::set(%d, %d, %u)", *pFormat, *pChannels, *pRate);
    if (mFdin >= 0) {
    ...
    if(vr_enable && dualmic_enabled) {
        ...
        audpre_mask = FLUENCE_ENABLE;
            ...
            ALOGD("enable fluence");
            if (ioctl(mFdin, AUDIO_ENABLE_AUDPRE, &audpre_mask)) {
                ALOGD("cannot write audio config");
                goto Error;
            }
        }
#endif
...
AudioHardware::AudioStreamInMSM8x60::~AudioStreamInMSM8x60()
{
    ALOGD("AudioStreamInMSM8x60 destructor");
    standby();
}
...
{
    unsigned short dec_id = INVALID_DEVICE;
    ALOGD("AudioStreamInMSM8x60::read(%p, %d)", buffer, bytes);
    if (!mHardware) return -1;
    ...
    if (mState < AUDIO_INPUT_OPENED) {
...
#endif
        {
            ...
            if (!(mChannels & AudioSystem::CHANNEL_IN_VOICE_DNLINK ||
                  mChannels & AudioSystem::CHANNEL_IN_VOICE_UPLINK)) {
                 ALOGD("dec_id = %d,cur_tx= %d",dec_id,cur_tx);
                 if(cur_tx == INVALID_DEVICE)
...
status_t AudioHardware::AudioStreamInMSM8x60::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    ALOGD("AudioStreamInMSM8x60::setParameters() %s", keyValuePairs.string());

    if (param.getInt(String8("vr_mode"), mForVR) == NO_ERROR)
        ALOGD("voice_recognition=%d", mForVR);
...
String8 AudioHardware::AudioStreamInMSM8x60::getParameters(const String8& keys)
{
    ...
    ALOGD("AudioStreamInMSM8x60::getParameters() %s", param.toString().string());
    return param.toString();
}
