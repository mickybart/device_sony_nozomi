...
const char *Volume::BLKID_PATH = "/system/xbin/blkid";
...
int Volume::formatVol(bool wipe) {
    ...
    extractMetadataType(devicePath);
    
    /* If the device has no filesystem, let's default to vfat.
     * A NULL mType will cause a MAPERR in the format
     * switch below */
    if (mType == NULL) {
        setType("vfat");
    }
...
/*
 * Use blkid to extract type from device.
 */
int Volume::extractMetadataType(const char* devicePath) {
    int res = 0;
