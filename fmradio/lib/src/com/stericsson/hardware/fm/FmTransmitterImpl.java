 ...
 * @hide
 */
class FmTransmitterImpl extends FmTransmitter {
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
    	public void onServiceConnected(ComponentName className, IBinder service) {
    		...
    		try {
    			mService = (IFmTransmitter) IFmTransmitter.Stub.asInterface(service);
    		} catch (Throwable t) {
    			mService = null;
    		}
    		if (mService == null) {
    ...
    FmTransmitterImpl(Context ctx, onServiceAvailableListener cb) {
    	mServiceAvailableCb = cb;
		if (!ctx.bindService(new Intent(IFmTransmitter.class.getName()),
                              mConnection, Context.BIND_AUTO_CREATE)) {
			Log.e(TAG, "Could not bind to IFmTransmitter Service");
        }
    }
